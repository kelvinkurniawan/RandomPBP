...
<div class="row no-gutters">
    ...
    <div class="col-md-3 bg-randomize-3 card widget right d-none d-sm-none d-md-block">
        <div class="container text-center sticky-top float-component">
            <div class="mt-3">
                <img class="rounded-circle" src="{{get_images_path(getUserById($row->followId, 'photo'))}}" width="50%">
            </div>
            <div class="font-weight-bold mt-3 text-dark">{{getUserById($row->followId, "name")}}</div>
            <div class="mt-1 mb-3 text-dark d-flex justify-content-center align-items-center small">
                ...
                <div class="ml-1"><em>{{getUserById($row->followId, "birth")}}</em></div>
            </div>
            <div class="font-weight-light text-dark small mt-1">
                {{getUserById($row->followId, "bio")}}
            </div>
            <div><a href="" class="btn btn-randomize btn-ghost mt-5 w-50" data-toggle="modal" data-target="#exampleModal">View Profile</a></div>
